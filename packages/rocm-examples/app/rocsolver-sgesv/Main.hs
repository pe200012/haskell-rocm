...
import ROCm.RocSOLVER (rocsolverSgesv)
...
run = do
  ...
  if "gfx1103" `isPrefixOf` archName && hsaOverride /= Just "11.0.0"
    then do
      putStrLn ("rocSOLVER sgesv: skipped on " <> archName <> " because this install only ships gfx1100 kernels.")
      putStrLn "Run with: HSA_OVERRIDE_GFX_VERSION=11.0.0 cabal run rocsolver-sgesv"
        ...
        bracket (mallocArray (n * nrhs) :: IO (Ptr CFloat)) free $ \hB ->
          bracket (mallocArray 1 :: IO (Ptr RocblasInt)) free $ \hInfo -> do
            ...
            when (not (infoOk && approxVec out expected)) $ do
              putStrLn "rocsolver_sgesv mismatch"
              putStrLn ("expected: " <> show expected)
              putStrLn ("got:      " <> show out)
              putStrLn ("info:     " <> show infoVals)
              exitFailure

      putStrLn "rocSOLVER sgesv: OK"
