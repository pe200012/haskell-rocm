...
import ROCm.RocSOLVER (rocsolverSgesdd)
...
run = do
  ...
  if "gfx1103" `isPrefixOf` archName && hsaOverride /= Just "11.0.0"
    then do
      putStrLn ("rocSOLVER sgesdd: skipped on " <> archName <> " because this install only ships gfx1100 kernels.")
      putStrLn "Run with: HSA_OVERRIDE_GFX_VERSION=11.0.0 cabal run rocsolver-sgesdd"
            ...
            bracket (mallocArray (n * n) :: IO (Ptr CFloat)) free $ \hV ->
              bracket (mallocArray 1 :: IO (Ptr RocblasInt)) free $ \hInfo -> do
                ...
                when (not (infoOk && approxVecWithTol 1.0e-3 sVals expectedS && approxVecWithTol 1.0e-3 recon aOriginal && approxVecWithTol 1.0e-3 uGram identity2 && approxVecWithTol 1.0e-3 vGram identity2)) $ do
                  putStrLn "rocsolver_sgesdd mismatch"
