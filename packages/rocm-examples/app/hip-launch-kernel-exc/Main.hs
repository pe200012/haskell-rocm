      ...
      bracket (hipMallocBytes bytes :: IO (DevicePtr CFloat)) hipFree $ \dIn ->
        bracket (hipMallocBytes bytes :: IO (DevicePtr CFloat)) hipFree $ \dOut ->
          bracket hipStreamCreate hipStreamDestroy $ \stream ->
            withDirectAddOneKernelAddress "hip_launch_kernel_exc_example" $ \kernelAddress -> do
                ...
                with pIn $ \pArgIn ->
                  with nArg $ \pArgN ->
                    withArray [castPtr pArgOut, castPtr pArgIn, castPtr pArgN] $ \kernelParams -> do
                      ...
                      if output == expected
                        then putStrLn "hip launch kernel exc: OK"
                        else error ("hip launch kernel exc mismatch: expected=" <> show expected <> ", got=" <> show output)
    ...
    ExitFailure code ->
      error
        ( "hipcc failed with exit code "
            <> show code
            <> " while building hipLaunchKernelExC helper\nstdout:\n"
            <> stdOut
            <> "\nstderr:\n"
            <> stdErr
        )
