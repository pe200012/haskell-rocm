...
run = withRocfft $ do
  let n = 16 :: Int
      ...
      bytes :: CSize
      bytes = fromIntegral (n * sizeOf (undefined :: Complex Float))
  ...
  bracket (mallocArray n) free $ \hIn ->
    bracket (mallocArray n) free $ \hOut -> do
      ...
      when (not (approxComplexVec out expected)) $ do
        putStrLn "rocFFT forward+inverse mismatch"
        putStrLn ("expected (input * n): " <> show expected)
        putStrLn ("got:               " <> show out)
        exitFailure

  putStrLn "rocFFT C2C 1D (forward+inverse): OK"
