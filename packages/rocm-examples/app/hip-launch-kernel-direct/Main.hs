      ...
      bracket (hipMallocBytes bytes :: IO (DevicePtr CFloat)) hipFree $ \dIn ->
        bracket (hipMallocBytes bytes :: IO (DevicePtr CFloat)) hipFree $ \dOut ->
          bracket hipStreamCreate hipStreamDestroy $ \stream ->
            withDirectAddOneKernelAddress "hip_launch_kernel_direct_example" $ \kernelAddress -> do
                ...
                with pIn $ \pArgIn ->
                  with nArg $ \pArgN ->
                    withArray [castPtr pArgOut, castPtr pArgIn, castPtr pArgN] $ \kernelParams -> do
                      ...
                      if output == expected
                        then putStrLn "hip direct kernel launch: OK"
                        else error ("hip direct kernel launch mismatch: expected=" <> show expected <> ", got=" <> show output)
