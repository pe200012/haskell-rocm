...
import ROCm.RocSOLVER (rocsolverSgesvdxBatched)
...
run = do
  ...
  if "gfx1103" `isPrefixOf` archName && hsaOverride /= Just "11.0.0"
    then do
      putStrLn ("rocSOLVER sgesvdx batched value-range: skipped on " <> archName <> " because this install only ships gfx1100 kernels.")
      putStrLn "Run with: HSA_OVERRIDE_GFX_VERSION=11.0.0 cabal run rocsolver-sgesvdx-batched-value"
                  ...
                  bracket (mallocArray (batchCount * strideFCount) :: IO (Ptr RocblasInt)) free $ \hIfail ->
                    bracket (mallocArray batchCount :: IO (Ptr RocblasInt)) free $ \hInfo -> do
                      ...
                      when (not (all fst reports)) $ do
                        putStrLn ("rocsolver_sgesvdx_batched value-range mismatch: " <> intercalate "; " (map snd reports))
                        exitFailure

      putStrLn "rocSOLVER sgesvdx batched value-range: OK"
