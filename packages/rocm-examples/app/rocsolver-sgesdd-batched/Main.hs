...
import ROCm.RocSOLVER (rocsolverSgesddBatched)
...
run = do
  ...
  if "gfx1103" `isPrefixOf` archName && hsaOverride /= Just "11.0.0"
    then do
      putStrLn ("rocSOLVER sgesdd batched: skipped on " <> archName <> " because this install only ships gfx1100 kernels.")
      putStrLn "Run with: HSA_OVERRIDE_GFX_VERSION=11.0.0 cabal run rocsolver-sgesdd-batched"
              ...
              bracket (mallocArray (batchCount * strideVCount) :: IO (Ptr CFloat)) free $ \hV ->
                bracket (mallocArray batchCount :: IO (Ptr RocblasInt)) free $ \hInfo -> do
                  ...
                  when (not (all fst reports)) $ do
                    putStrLn ("rocsolver_sgesdd_batched mismatch: " <> intercalate "; " (map snd reports))
                    exitFailure

      putStrLn "rocSOLVER sgesdd batched: OK"
